        ...
        while(1) 
	{
		...
		if(task_iter >= 1) //piping present
		{
			...
			for(i = 0; i < task_iter + 1; i++)
			{
				if((pid[i] = fork()) == 0)
				{
					...
					for(j = 0; j < 3; j++)
						...
						{
							/*if(j == 0) //close output nozzle of input pipe
								close(tasks[i].fd_redir[0]+1);
							if(j == 1) //close input nozzle of output pipe
								close(tasks[i].fd_redir[1]-1);*/
							dup2(tasks[i].fd_redir[j], j);
							//close(tasks[i].fd_redir[j]); //close other nozzle
						}
					int k;
			...
			for(i = 0; i < task_iter + 1; i++)
			{
				...
				waitpid(pid[i], &status, WUNTRACED | WNOHANG);
